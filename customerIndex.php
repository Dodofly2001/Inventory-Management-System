...
/*if(!isset($_SESSION['bookingID']))
    {
      echo "<script>alert('Please create new order first before start booking product'); window.location = 'StartNewOrder.php';</script>";
    }
*/
function checkBookingID($conn,$customerID)
{
  $sql = "SELECT Booking_ID FROM booking WHERE Customer_ID = '".$customerID."'";
...
if(isset($_SESSION['bookingID']))
{
  ...
  {
    echo "<script>alert('Please select any of the product');</script>";
  }
  
}
else if(checkBookingID($conn,$_SESSION['Customer_ID']) == true)
{
  echo "<script>alert('Please continue your previous booking')</script>";
  ...
  if($row > 0)
  {
  $_SESSION['bookingID'] = $value['BOOKINGID'];
  }
              ...
              <div class="card-body">
                ...
                <p class="card-text">
                  ...
                  <script>
                    ...
                    {
                      <?php 
                          /*$totaladv = "SELECT COUNT(Audit_ID) AS total FROM auditview";
                          $result1 = mysqli_query($conn,$totaladv);
                          $value = mysqli_fetch_assoc($result1);
                          $num_rows = $values['total'];
                          $increment = $num_rows + 1;
                          $auditView = "ADV".$increment;
                          ...
                          $sql = "INSERT INTO auditview(AuditView_ID,Customer_ID,Product_ID,ViewsDateTime) VALUES ('".$auditView."','".$_SESSION['Customer_ID']."', '".$productID."' , '".$date."')";
                      */
                      ?>
                    }
                  </script>
                  ...
                  <?php
                    ...
                    { 
                      ...
                      echo "<form action = 'Productdesc.php' method = 'POST'>";
